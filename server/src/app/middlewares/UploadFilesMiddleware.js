...
const diskStorage = multer.diskStorage({
  ...
  filename: (req, file, callback) => {
    ...
    const fileName = `${Date.now()}-tana-${slug(file.originalname.replace(/\.[^/.]+$/, ""))}.${file.originalname.split(".").pop()}`;
    callback(null, fileName);
  },
});

const filter = (req, file, callback) => {
  const math = ["image/png", "image/jpeg", "application/msword", "application/pdf",
    "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "video/mp4", "video/x-matroska", "audio/mp3",
