...
class CommentController {
	...
	async react(req, res, next) {
		try {
			const comment = await Comment.findById(req.params.id);
			if (comment) {
				const reactOfUser = await React.findOne({
					user: req.user._id,
				...
				if (reactOfUser && reactOfUser.type.toString() === req.body.type.toString()) {
					...
					await React.findByIdAndUpdate(reactOfUser._id, {
	...
	async delete(req, res, next) {
		const comment = await Comment.findById(req.params.id).populate('author', '_id fullname profilePicture');
		const post = await Post.findById(req.params.postId);
		if (comment) {
			if (
				...
				if (index > -1) {
					...
					post.lastestFiveComments = await Comment.find({
				...
				const numberReplyDeleted = await Comment.deleteMany({
					replyTo: req.params.id,
				});
				//decrease number of comments of post
				await Post.findByIdAndUpdate(req.params.postId, {
					$inc: {
						numberComment: -1 - numberReplyDeleted.deletedCount,
					},
				});
