...
class UserController {
				...
				if (currentUser.friends.some((friend) => friend.user.toString() === req.params.id)) {
					...
					if (
						...
						await user.updateOne({
							$push: {
								friendRequests: { user: currentUser._id },
								followers: { user: currentUser._id },
							},
						});
						await currentUser.updateOne({
							$push: {
								sentRequests: { user: user._id },
		...
		if (req.params.id.toString() !== req.user._id.toString()) {
			try {
				...
				if (
					...
					await user.updateOne({
						$pull: { sentRequests: { user: currentUser._id } },
					});
					await user.updateOne({
						$push: {
		...
		if (req.params.id.toString() !== req.user._id.toString()) {
			try {
				...
				if (
					...
					await user.updateOne({
						$pull: {
							sentRequests: { user: currentUser._id },
							followings: { user: currentUser._id },
						},
					});
					await currentUser.updateOne({
	...
	async getFriendsList(req, res, next) {
		try {
			...
			let query = [
				{
					_id: { $in: listIDFirends },
					$sort: { 'friends.date': sort },
				},
			];
			if (req.query.gender) {
			...
			if (type === 'all') {
				...
				query = [
					{ $match: { _id: { $in: listFriendsOfUser } } },
					{
						$addFields: {
							ListUsers: {
								$indexOfArray: [listFriendsOfUser, '$_id'],
							},
						},
					},
					{ $sort: { ListUsers: 1 } },
				...
				query = [
					{ $match: { _id: { $in: listRequestOfUser } } },
					{
						$addFields: {
							ListUsers: {
								$indexOfArray: [listRequestOfUser, '$_id'],
							},
						},
					},
					{ $sort: { ListUsers: 1 } },
	...
	async getFriendsOfFriends(req, res, next) {
		try {
			...
			let query = [{ _id: { $in: friendsOfFriendsFilter } }];
