...
class MessageController {
    ...
    async update(req, res, next) {
        try {
            ...
            if (!message.reader.includes(req.user._id))
                message.reader.push(req.user._id);
            await message.save();
            res.status(200).json(message);
        } catch (err) {
            console.log(err);
    ...
    async fetchMessages(req, res, next) {
        ...
        if (conversation.members.some(mem => mem.user.toString() === req.user._id.toString())) {
            Message.paginate({ conversation: req.params.conversationId, createdAt: { $gte: deletedDate }, },
                {
